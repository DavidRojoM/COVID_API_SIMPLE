...
fetch(`${API_URL}/cases?country=Spain`)
	...
	.then((communities) => {
		toArray(communities).forEach((comunity) => {
			...
			cardContentH2.appendChild(document.createTextNode(comunity.comunity));
			...
			info1p.appendChild(document.createTextNode(comunity.confirmed));
			...
			info2p.appendChild(document.createTextNode(comunity.recovered));
			...
			info3p.appendChild(document.createTextNode(comunity.deaths));
...
function toArray(communities) {
	let array = [];
	for (const comunity in communities) {
		if (
			Object.hasOwnProperty.call(communities, comunity) &&
			comunity != "Unknown"
		) {
			array.push({
				comunity: `${comunity}`,
